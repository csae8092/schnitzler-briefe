...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="tei" version="2.0">
    ...
    <xsl:variable name="entity" as="node()">
        <xsl:choose>
            <xsl:when test="not(empty(//tei:place[replace(@xml:id,'pmb','')=replace($entiyID, 'pmb','')][1]))">
                <xsl:value-of select="//tei:place[replace(@xml:id,'pmb','')=replace($entiyID, 'pmb','')][1]"/>
            </xsl:when>
            <xsl:otherwise>
                    ...
                    <div class="modal-content">
                        <xsl:choose>
                            <xsl:when test="$entity">
                                <xsl:variable name="entity" select="//tei:place[replace(@xml:id,'pmb','')=replace($entiyID, 'pmb','')]"/>
